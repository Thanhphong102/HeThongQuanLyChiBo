...
const DEFAULT_ADMIN_URL = 'https://admin-frontend-chibo.vercel.app';
...
const getFrontendUrl = (portal) => {
    if (portal === 'user') {
        return process.env.USER_FRONTEND_URL || DEFAULT_USER_URL;
    }
    return process.env.ADMIN_FRONTEND_URL || process.env.FRONTEND_URL || DEFAULT_ADMIN_URL;
};
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f3f0e9;">
        <tr><td align="center" style="padding:28px 12px;">
            <table role="presentation" class="email-shell" width="600" cellspacing="0" cellpadding="0" border="0" style="width:600px;max-width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 12px 34px rgba(91,23,26,.12);">
                <tr><td class="email-header" style="padding:28px 34px;background:#a91f23;border-bottom:5px solid #f1cf50;">
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr>
                        <td width="52" valign="middle"><div style="width:42px;height:42px;line-height:42px;text-align:center;border-radius:10px;background:#fff1aa;color:#a91f23;font-size:24px;font-weight:700;">★</div></td>
                        <td valign="middle" style="color:#ffffff;"><div style="font-size:16px;font-weight:700;line-height:1.35;">Hệ thống Quản lý Chi bộ</div><div style="margin-top:3px;font-size:12px;color:#fff1aa;letter-spacing:.4px;">TRƯỜNG ĐẠI HỌC KỸ THUẬT – CÔNG NGHỆ CẦN THƠ</div></td>
                    </tr></table>
                </td></tr>
                <tr><td class="email-body" style="padding:34px 38px 38px;">
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 22px;background:#faf8f3;border:1px solid #e7dfd3;border-radius:10px;border-collapse:separate;overflow:hidden;">${detailRows}</table>
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 26px;background:#fff8d9;border-left:4px solid #e4bd2f;border-radius:6px;"><tr><td style="padding:14px 16px;color:#5c4712;font-size:14px;line-height:1.6;"><strong>Lưu ý:</strong> ${escapeHtml(notice)}</td></tr></table>
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td align="center" style="padding:2px 0 18px;"><a class="email-button" href="${safeUrl}" target="_blank" style="display:inline-block;padding:14px 26px;background:#f1cf50;color:#8b1517;text-decoration:none;border-radius:8px;font-size:15px;font-weight:700;line-height:1.2;box-shadow:0 5px 14px rgba(169,31,35,.18);">${escapeHtml(buttonText)} →</a></td></tr></table>
                ...
                <tr><td style="padding:17px 24px;background:#2c2521;color:#d8d0c8;text-align:center;font-size:11px;line-height:1.55;">Đây là email tự động. Vui lòng không trả lời hoặc chia sẻ thông tin đăng nhập trong email này.</td></tr>
            </table>
        </td></tr>
    </table>
</body>
...
const sendAccountCreationEmail = async (email, hoTen, username, password, options = {}) => {
    const portal = options.portal === 'user' ? 'user' : 'admin';
    const roleLabel = portal === 'admin' ? 'Chi ủy' : 'Đảng viên';
    const loginUrl = joinUrl(getFrontendUrl(portal), '/login');
    const subject = `Tài khoản ${roleLabel} của đồng chí đã được cấp`;

    return sendEmail({
        to: email,
        subject,
        text: `Kính gửi đồng chí ${hoTen},\n\nTài khoản ${roleLabel} đã được cấp.\nTên đăng nhập: ${username}\nMật khẩu tạm: ${password}\n\nĐăng nhập tại: ${loginUrl}\nVui lòng đổi mật khẩu ngay trong lần đăng nhập đầu tiên.`,
        html: buildEmailHtml({
            preview: `Tài khoản ${roleLabel} đã được cấp. Xem thông tin đăng nhập và mở hệ thống.`,
            label: 'CẤP TÀI KHOẢN',
            title: 'Tài khoản của đồng chí đã sẵn sàng',
            greeting: `Kính gửi đồng chí ${hoTen},`,
            message: `Tài khoản ${roleLabel} của đồng chí đã được tạo trên hệ thống.`,
            details: [
                { label: 'Cổng truy cập', value: roleLabel },
            ...
            buttonText: `Mở cổng ${roleLabel}`,
            buttonUrl: loginUrl
        })
    });
};

const sendPasswordResetEmail = async (email, hoTen, username, newPassword, options = {}) => {
    const portal = options.portal === 'user' ? 'user' : 'admin';
    const roleLabel = portal === 'admin' ? 'Chi ủy' : 'Đảng viên';
    const loginUrl = joinUrl(getFrontendUrl(portal), '/login');
    const subject = `Mật khẩu tài khoản ${roleLabel} đã được cấp lại`;

    return sendEmail({
        to: email,
        subject,
        text: `Kính gửi đồng chí ${hoTen},\n\nMật khẩu tài khoản ${roleLabel} đã được cấp lại.\nTên đăng nhập: ${username}\nMật khẩu tạm mới: ${newPassword}\n\nĐăng nhập tại: ${loginUrl}\nVui lòng đổi mật khẩu ngay sau khi đăng nhập.`,
        html: buildEmailHtml({
            preview: `Mật khẩu tạm mới của tài khoản ${roleLabel} và đường dẫn đăng nhập.`,
            label: 'KHÔI PHỤC TÀI KHOẢN',
            title: 'Mật khẩu của đồng chí đã được cấp lại',
            greeting: `Kính gửi đồng chí ${hoTen},`,
            message: `Hệ thống đã tạo mật khẩu tạm mới cho tài khoản ${roleLabel} của đồng chí.`,
            details: [
                { label: 'Cổng truy cập', value: roleLabel },
                { label: 'Tên đăng nhập', value: username },
